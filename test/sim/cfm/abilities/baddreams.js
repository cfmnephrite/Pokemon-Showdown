...
describe('CFM - Bad Dreams', function () {
	...
	it('The power of Nightmare is boosted by 50%', function () {
		...
		assert.ok(battle.p1.active[0].hp > battle.p1.active[0].maxhp / 2); // W/o Bad Dreams, does less than 50%
		assert.ok(battle.p2.active[0].hp < battle.p2.active[0].maxhp / 2); // With, does more
	});
	...
	it('The power of Never-Ending Nightmare is boosted by 50%', function () {
		...
		assert.ok(battle.p1.active[0].hp > 0.4 * battle.p1.active[0].maxhp); // W/o Bad Dreams, does less than 60%
		assert.ok(battle.p2.active[0].hp < 0.3 * battle.p2.active[0].maxhp); // With, does more than 70%
	});
});
