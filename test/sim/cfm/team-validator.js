...
describe('CFM - Team Validator', function () {
	it('CFM should be a valid format', function () {
		try {
			Dex.formats.getRuleTable(Dex.formats.get('gen9cfmou'));
		} catch (e) {
			e.message = `gen9cfmou: ${e.message}`;
			throw e;
		}
	}).timeout(10000);
	it('should allow for CFM learnsets', function () {
		...
		const illegal = TeamValidator.get('gen9cfmou').validateTeam(team);
		assert(!illegal);
	}).timeout(10000);
	...
	it('CFM complex bans', function () {
		...
		let illegal = TeamValidator.get('gen9cfmou').validateTeam(team);
		assert(illegal);
		...
		illegal = TeamValidator.get('gen9cfmou').validateTeam(team);
		assert(illegal);
